...
function customEntries(obj) {
  ...
  if (typeof obj !== "object") {
    console.log("Помилка: Аргумент не є об'єктом.");
  }
...
function customObjectFromEntries(entries) {
  ...
  const Obj = Array.isArray(entries)
    ? entries
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const Freez = Object.freeze(Add);
  // Перевіряємо, чи об'єкт "заморожено"
  if (!Object.isFrozen(Freez)) {
    console.log("об'єкт не було заморожено");
  }
  return Freez;
...
function getObjectValuesSum(obj) {
  ...
  const init = 0;
  const Sum = Val.reduce((elem1, elem2) => elem1 + elem2, init);
  // Повертаємо суму
  return Sum;
}
...
function convertArrayToObj(arr) {
  ...
  for (let i = 0; i >= arr.length; i++) {
    for (const [key, value] of Object.entries(arr)) {
      if (a.hasOwnProperty(key, value)) {
        console.log(`У масиві є дубльований ключ: ${key}`);
      } else {
        a.Object.fromEntries(key, value);
      }
    }
    return a;
  }
